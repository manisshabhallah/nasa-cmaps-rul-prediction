...
IMPORTANT FIXES (vs. original version):
  ...
  FIX 2 - Metric mismatch (the bigger issue): Even after fixing #1, Val RMSE
  stayed ~11-12 while test RMSE stayed ~28. Why? Because RUL labels are
  piecewise-capped at 125, most cycles in an engine's life sit on the flat
  "healthy" plateau (RUL=125) - these windows are trivially easy to predict.
  The original validation metric averaged over ALL windows of each val
  engine, so it was dominated by these easy plateau windows. The TEST
  metric, by contrast, only evaluates the LAST window per engine (the
  hardest, most failure-relevant one). Averaging over all windows vs.
  scoring only the last window are two different metrics - they were never
  going to agree.

  Fix: validation now also scores ONLY the last window per val-engine
  (same protocol as test). This makes Val RMSE during training an honest,
  apples-to-apples preview of test RMSE, and early stopping now selects the
  checkpoint that is actually best for the metric that matters.
...
def train_model(dataset_id: str = "FD001",
    ...
    # -------- FIX 2: score validation the SAME way test is scored --------
    # Only the LAST window per val-engine is used for the RMSE that drives
    # early stopping / checkpoint selection. Sequences are appended in
    # increasing-cycle order per engine (see gold_layer.create_sequences),
    # so the highest index seen for a given engine_id is its last window.
    eid_to_last_idx = {}
    for idx, eid in enumerate(engine_ids):
        eid_to_last_idx[int(eid)] = idx   # last write wins -> last (max-cycle) window

    val_engines = sorted(set(engine_ids[val_idx].tolist()))
    val_last_idx = np.array([eid_to_last_idx[int(e)] for e in val_engines])

    val_ds = Subset(dataset, val_last_idx)   # test-like: 1 sample per val engine

    train_size = len(train_idx)
    val_size   = len(val_last_idx)
    ...
    print(f"  Train samples: {train_size}  |  Val samples: {val_size} "
          f"(1 last-window sample per val engine, test-like)")
    ...
    print(f"  (Val RMSE = last-window-per-engine on unseen val engines - "
          f"same protocol as test, so this should now track test RMSE closely.)")
